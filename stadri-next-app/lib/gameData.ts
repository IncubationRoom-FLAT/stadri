...
export function assignOdai(playerCount: number): { cat: string; prob: string }[] {
    let globalOdaiPool: { cat: string; prob: string }[] = [];
    Object.keys(ODAIS).forEach(cat => {
        (ODAIS as any)[cat].forEach((prob: string) => {
            globalOdaiPool.push({ cat, prob });
        });
    });
    globalOdaiPool.sort(() => Math.random() - 0.5);
    return Array.from({ length: playerCount }, () => globalOdaiPool.pop()!);
}
