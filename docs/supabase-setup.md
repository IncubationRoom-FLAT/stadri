...
### `state` カラムの JSON 構造について

`state` カラムは JSONB 型で、ゲームの全状態を 1 つの JSON オブジェクトとして格納します。
スキーマ変更なしにフィールドを追加できるため、**テーブル定義は将来にわたって変更不要**です。

現在格納されている主なフィールド（`lib/types.ts` の `MultiRoomState` を参照）:

| フィールド | 型 | 説明 |
|---|---|---|
| `phase` | string | ゲームフェーズ（`waiting` / `pitch` / `invest` 等） |
| `players` | array | 参加プレイヤー一覧 |
| `stamps` | array | プレゼン中に送られたスタンプ履歴（最大 100 件） |
| `invLog` | array | 投資ログ |
| ...他 | | タイマー情報・ランキング等 |

> **スタンプ機能について**: `stamps` フィールドはスタンプ機能の追加により導入されました。
> 既存ルームデータには `stamps` が存在しない場合がありますが、API 側で `state.stamps ?? []` として
> フォールバック処理しているため、**マイグレーション SQL は不要**です。
